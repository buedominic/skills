...
Spec-to-Implementation-Pipeline für Codex und Claude Code: **Brainstorm → Spec →
...
|---|---|---|
| Skill `$spec-to-implementation` / `/spec-to-implementation` | `skills/spec-to-implementation/SKILL.md` | Dünner Orchestrator: Stufen, gebündelte Gates, Manifest-Pflege, Weichen-Erkennung, Light-Mode |
...
| Referenz: Codex-Runtime | `skills/spec-to-implementation/references/codex-runtime.md` | Capability-Preflight, begrenzter Rollen-Pool, Thread-Close/Reuse und Artefaktprüfung |
| Referenz: Task-Ledger | `skills/spec-to-implementation/references/progress-ledger.md` | Nutzt `.superpowers/sdd/progress.md` taskgenau für Resume/Compaction in Stage 5 |
...
1. **`AGENTS.md` / `CLAUDE.md`** — Konventionen, Verifikations-Befehle,
   Branch-Strategie. Codex priorisiert `AGENTS.md`, Claude `CLAUDE.md`.
2. **`.codex/workflow.config.json` / `.claude/workflow.config.json`**
   (optional) — Spec-/Plan-Verzeichnisse,
...
- **Codex-Subagenten** — in Codex der native Review-/Implementierungsweg. In
  Claude kann Codex MCP (`mcp__codex__codex`) weiterhin als Zweitmodell dienen.
